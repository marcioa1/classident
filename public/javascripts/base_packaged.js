...
$("#cheques_ids").val(selecionados);var total_a_pagar=$("#pagamento_valor_pago_real").val();if(total_a_pagar<total_de_cheques){alert("A soma dos valores dos cheques selecionados é maior que o valor do pagamento.");}
...
function verifica_se_tem_paciente_com_este_nome(){$.ajax({url:"verifica_nome_do_paciente/0",data:{nome:$("#paciente_nome").val()},type:"GET",success:function(data){$("#transfere_clinica").html(data.paciente.complemento);$("#transfere").dialog({resizable:false,height:140,modal:true,buttons:{"Transfere paciente ":function(){transfere_paciente(data.paciente.id);},"Cancela":function(){$(this).dialog("close");}}});}});}
function transfere_paciente(id){$.ajax({url:"transfere_paciente",data:{id:id},type:"GET",success:function(data){location.href="0/abre?nome="+data}});}
