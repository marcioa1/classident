class Converte
  ...
  def clinica_conta_bancaria
    abre_arquivo_de_erros('Conta Bancária')
    puts "Convertendo contas bancárias ..."
    ContaBancaria.delete_all
    FasterCSV.foreach('doc/convertidos/conta_bancaria.txt', option={:col_sep=>';'}) do |registro|
        clinica_id = CLINICAS[registro.last]
        ContaBancaria.create(:sequencial => registro[0], 
                             :nome       => registro[1], 
                             :clinica_id => clinica_id)
      end
    fecha_arquivo_de_erros('Conta Bancária')
  end
    ...
    FasterCSV.foreach('doc/convertidos/recebimento.txt', option={:col_sep=>';'}) do |registro|
      begin
        ...
        if forma_recebimento.nil?
          r.formas_recebimento_id = forma_inexistente.id
        else
  ...
  def pagamento
    ...
    FasterCSV.foreach('doc/convertidos/pagamento.txt', option={:col_sep=>';'}) do |registro|
      begin
        clinica_id          = id_da_clinica(registro.last.downcase)
        t                   = Pagamento.new
        t.clinica_id        = clinica_id
        tipo_pagamento      = TipoPagamento.find_by_seq_and_clinica_id(registro[1].to_i,clinica_id, :select=>['id'])
        ...
        t.valor_restante    = le_valor(registro[14]) unless registro[14].blank?
        ...
        if registro[13].to_i > 0
          seq = registro[13].to_i
          conta_bancaria      = ContaBancaria.find_by_sequencial_and_clinica_id(seq, clinica_id)
          t.conta_bancaria_id =  conta_bancaria.id unless conta_bancaria.nil? 
        end
  ...
  def clinica_cheque
    ...
    FasterCSV.foreach('doc/convertidos/cheque.txt', option={:col_sep=>';'}) do |registro|
      begin
        ...
        if registro[1].to_i > 0 
          t.banco_id           = BANCOS[registro[1].to_i] #Banco.find_by_numero(registro[1].to_i)
        end
        ...
        if registro[9].to_i > 0
          d = Destinacao.find_by_sequencial_and_clinica_id(registro[9].to_i,clinica_id)
          if d.present?
        ...
        if registro[11].to_i > 0
          # if registro[11].to_i == 94416 then
            # debugger
            pagamento             = Pagamento.find_by_sequencial_and_clinica_id(registro[11].to_i, clinica_id,
              :select=>'id')
        ...
        t.save
        recebimentos = Recebimento.all(:conditions => ['sequencial_cheque = ? and clinica_id = ? ', t.sequencial, clinica_id],
  ...
  def adm_destinacao
    abre_arquivo_de_erros('Destinação Administração')
    puts "Convertendo destinação na administração ...."
    clinica = ''
    FasterCSV.foreach('doc/convertidos/adm_destinacao.txt', option={:col_sep=>';'}) do |registro|
      begin
        clinica_id   = 1
        t            = Destinacao.new
        t.clinica_id = 1
    ...
    fecha_arquivo_de_erros("Destinação Administração")
  end
  
  def adm_cheques
    abre_arquivo_de_erros('Cheques na Administração')
    ...
    segunda  = 0
    terceira = 0
    puts "Criando indíces na tabela cheques ..."
    # ActiveRecord::Migration.add_index :pagamentos, [:clinica_id, :sequencial], :name =>'clinica_id_sequencial'
    # ActiveRecord::Migration.add_index :cheques, [:clinica_id, :numero, :agencia], :name => 'clinica_id_numero_agencia'
    puts "Lendo arquivo txt ..."
    FasterCSV.foreach('doc/convertidos/adm_cheque.txt', option={:col_sep=>';'}) do |registro|
      begin
        ct += 1
        debugger
        if @siglas.include?(registro[1].downcase)
          clinica_id       = id_da_clinica(registro[1].downcase) #@as_clinicas.find{ |cl| cl.sigla == registro[1].downcase}
          seq              = registro[2].to_i
          ...
          adm              = registro[26]
          agencia          = registro[4]
          num_cheque       = registro[6]
          ch               = Cheque.find_by_clinica_id_and_numero_and_agencia(clinica_id, num_cheque, agencia)
          if ch.nil?
            conta_corrente = registro[5]
            ch = Cheque.find_by_clinica_id_and_numero_and_conta_corrente(clinica_id, num_cheque, conta_corrente)
            if ch.nil?
              valor = le_valor(registro[8])
              ch    = Cheque.find_by_clinica_id_and_numero_and_valor(clinica_id, num_cheque, conta_corrente)
              terceira += 1
            else
              segunda += 1
            end
          ...
          if ch && ch.entregue_a_administracao
            ...
            ch.destinacao_id    = destinacao.id
            ch.data_destinacao  = data_destinacao
            if seq_pagamento > 0
              reg_pagamento     = Pagamento.find_by_clinica_id_and_sequencial(1, seq_pagamento, :select => 'id')
            ...
            if !registro[25].blank?
              AcompanhamentoCheque.create(:cheque_id => ch.id, :descricao => registro[25], :user_id => -1, :origem => 'administração')
            end
          end
        end
      rescue Exception => ex
    ...
    @@arquivo.puts "Primeira : #{primeira}"
    @@arquivo.puts "Segunda : #{segunda}"
    @@arquivo.puts "Terceira : #{terceira}"
  ...
  def adm_pagamento
    ...
    @@clinica = Clinica.administracao.first
    pgs = Pagamento.all(:conditions=>['clinica_id=1'], :select=>'id')
    pgs.each do |pag|
      pag.delete
    end
    FasterCSV.foreach('doc/convertidos/adm_pagamento.txt', option={:col_sep=>';'}) do |registro|
      begin
        ...
        if registro[12].to_i > 0
          seq = registro[12].to_i
          conta_bancaria      = ContaBancaria.find_by_sequencial_and_clinica_id(seq,1)
          t.conta_bancaria_id =  conta_bancaria.id unless conta_bancaria.nil? 
        end
  ...
  def adm_conta_bancaria
    abre_arquivo_de_erros('Conta Bancária Administração')
    puts "Convertendo contas bancárias ..."
    ContaBancaria.delete_all
    FasterCSV.foreach('doc/convertidos/adm_conta_bancaria.txt', option={:col_sep=>';'}) do |registro|
        ContaBancaria.create(:sequencial => registro[0], 
                             :nome       => registro[1], 
                             :clinica_id => 1)
    end
    fecha_arquivo_de_erros('Conta Bancária Administração')
  end
  ...
  def inicia_bancos_em_memoria
    Banco.all(:select=>('numero, id')).each do |b|
      BANCOS[b.numero.to_i] = b.id
    end
  end
